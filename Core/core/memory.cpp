...
private:
    bool* At(VAddr addr) {
        if (addr >= VRAM_VADDR && addr < VRAM_VADDR_END) {
            return &vram[(addr - VRAM_VADDR) / MANDARINE_PAGE_SIZE];
        }
        if (addr >= LINEAR_HEAP_VADDR && addr < LINEAR_HEAP_VADDR_END) {
            return &linear_heap[(addr - LINEAR_HEAP_VADDR) / MANDARINE_PAGE_SIZE];
        }
        if (addr >= NEW_LINEAR_HEAP_VADDR && addr < NEW_LINEAR_HEAP_VADDR_END) {
            return &new_linear_heap[(addr - NEW_LINEAR_HEAP_VADDR) / MANDARINE_PAGE_SIZE];
        }
        if (addr >= PLUGIN_3GX_FB_VADDR && addr < PLUGIN_3GX_FB_VADDR_END) {
            return &plugin_fb[(addr - PLUGIN_3GX_FB_VADDR) / MANDARINE_PAGE_SIZE];
        }
        return nullptr;
    }

    std::array<bool, VRAM_SIZE / MANDARINE_PAGE_SIZE> vram{};
    std::array<bool, LINEAR_HEAP_SIZE / MANDARINE_PAGE_SIZE> linear_heap{};
    std::array<bool, NEW_LINEAR_HEAP_SIZE / MANDARINE_PAGE_SIZE> new_linear_heap{};
    std::array<bool, PLUGIN_3GX_FB_SIZE / MANDARINE_PAGE_SIZE> plugin_fb{};
...
public:
    ...
    void ReadBlockImpl(const Kernel::Process& process, const VAddr src_addr, void* dest_buffer,
        ...
        std::size_t page_index = src_addr >> MANDARINE_PAGE_BITS;
    ...
    void WriteBlockImpl(const Kernel::Process& process, const VAddr dest_addr,
        ...
        auto& page_table = *process.vm_manager.page_table;
        std::size_t remaining_size = size;
        std::size_t page_index = dest_addr >> MANDARINE_PAGE_BITS;
...
void MemorySystem::MapPages(PageTable& page_table, u32 base, u32 size, MemoryRef memory,
    ...
    LOG_DEBUG(HW_Memory, "Mapping {} onto {:08X}-{:08X}", (void*)memory.GetPtr(),
              base * MANDARINE_PAGE_SIZE, (base + size) * MANDARINE_PAGE_SIZE);

    if (impl->system.IsPoweredOn()) {
        RasterizerFlushVirtualRegion(base << MANDARINE_PAGE_BITS, size * MANDARINE_PAGE_SIZE,
    ...
    while (base != end) {
        ...
        if (type == PageType::Memory && impl->cache_marker.IsCached(base * MANDARINE_PAGE_SIZE)) {
            page_table.attributes[base] = PageType::RasterizerCachedMemory;
            page_table.pointers[base] = nullptr;
        }

        base += 1;
        if (memory != nullptr && memory.GetSize() > MANDARINE_PAGE_SIZE)
...
void MemorySystem::UnmapRegion(PageTable& page_table, VAddr base, u32 size) {
    ASSERT_MSG((size & MANDARINE_PAGE_MASK) == 0, "non-page aligned size: {:08X}", size);
    ASSERT_MSG((base & MANDARINE_PAGE_MASK) == 0, "non-page aligned base: {:08X}", base);
    MapPages(page_table, base / MANDARINE_PAGE_SIZE, size / MANDARINE_PAGE_SIZE, nullptr,
             PageType::Unmapped);
}
...
T MemorySystem::Read(const VAddr vaddr) {
    const u8* page_pointer = impl->current_page_table->pointers[vaddr >> MANDARINE_PAGE_BITS];
    if (page_pointer) {
        // NOTE: Avoid adding any extra logic to this fast-path block
        T value;
        std::memcpy(&value, &page_pointer[vaddr & MANDARINE_PAGE_MASK], sizeof(T));
        return value;
    }
    ...
    PageType type = impl->current_page_table->attributes[vaddr >> MANDARINE_PAGE_BITS];
    switch (type) {
    case PageType::Unmapped:
...
void MemorySystem::Write(const VAddr vaddr, const T data) {
    u8* page_pointer = impl->current_page_table->pointers[vaddr >> MANDARINE_PAGE_BITS];
    if (page_pointer) {
        // NOTE: Avoid adding any extra logic to this fast-path block
        std::memcpy(&page_pointer[vaddr & MANDARINE_PAGE_MASK], &data, sizeof(T));
        return;
    }
...
bool MemorySystem::WriteExclusive(const VAddr vaddr, const T data, const T expected) {
    ...
    if (page_pointer) {
        const auto volatile_pointer =
            reinterpret_cast<volatile T*>(&page_pointer[vaddr & MANDARINE_PAGE_MASK]);
...
bool MemorySystem::IsValidVirtualAddress(const Kernel::Process& process, const VAddr vaddr) {
    ...
    auto page_pointer = page_table.pointers[vaddr >> MANDARINE_PAGE_BITS];
    if (page_pointer) {
        return true;
    }

    if (page_table.attributes[vaddr >> MANDARINE_PAGE_BITS] == PageType::RasterizerCachedMemory) {
        return true;
    }
...
u8* MemorySystem::GetPointer(const VAddr vaddr) {
    u8* page_pointer = impl->current_page_table->pointers[vaddr >> MANDARINE_PAGE_BITS];
    if (page_pointer) {
        return page_pointer + (vaddr & MANDARINE_PAGE_MASK);
    }

    if (impl->current_page_table->attributes[vaddr >> MANDARINE_PAGE_BITS] ==
    ...
    for (unsigned i = 0; i < num_pages; ++i, paddr += MANDARINE_PAGE_SIZE) {
        for (VAddr vaddr : PhysicalToVirtualAddressForRasterizer(paddr)) {
            ...
            for (auto& page_table : impl->page_table_list) {
                PageType& page_type = page_table->attributes[vaddr >> MANDARINE_PAGE_BITS];
                ...
                if (cached) {
                    ...
                    case PageType::Memory:
                        ...
                        page_table->pointers[vaddr >> MANDARINE_PAGE_BITS] = nullptr;
                        break;
                    default:
                        UNREACHABLE();
                    }
                    ...
                    case PageType::RasterizerCachedMemory: {
                        ...
                        page_table->pointers[vaddr >> MANDARINE_PAGE_BITS] =
                            GetPointerForRasterizerCache(vaddr & ~MANDARINE_PAGE_MASK);
                        break;
                    }
                    default:
                        UNREACHABLE();
