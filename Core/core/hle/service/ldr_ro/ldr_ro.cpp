...
void RO::LoadCRO(Kernel::HLERequestContext& ctx, bool link_on_load_bug_fix) {
    ...
    if (cro_buffer_ptr & Memory::MANDARINE_PAGE_MASK) {
    ...
    if (cro_address & Memory::MANDARINE_PAGE_MASK) {
    ...
    if (cro_size & Memory::MANDARINE_PAGE_MASK) {
