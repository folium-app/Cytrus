...
void RO::LoadCRO(Kernel::HLERequestContext& ctx, bool link_on_load_bug_fix) {
    ...
    if (cro_buffer_ptr & Memory::CYTRUS_PAGE_MASK) {
    ...
    if (cro_address & Memory::CYTRUS_PAGE_MASK) {
    ...
    if (cro_size & Memory::CYTRUS_PAGE_MASK) {
